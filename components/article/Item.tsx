...
const ArticleItem: FunctionComponent<Props> = ({ article, role }) => (
  <div
  ...
  >
    ...
    <div tw="mt-4 flex items-center">
      <Link href="/[user]" as={`/@${article.author.username}`} passHref>
        <a tw="flex-1 mr-4 text-xs text-gray-500 whitespace-no-wrap">
          Read More...
        </a>
      </Link>
      {article.tagList && (
        <div tw="-my-1" role="list">
          {article.tagList?.map((tag, index) => (
            <Link key={index} href={`/?tag=${tag}`} passHref>
              <a
                tw="ml-2 my-1 inline-block border border-gray-500 rounded-full px-3 py-1 text-xs text-gray-500 hover:border-green-600 hover:text-green-600"
                role="listitem"
              >
                {tag}
              </a>
